...
def main():
    manifest = {
        ...
        "note": "Images and audio are decoupled pools. `group` keys coherent triples "
                "(image timestamp / audio recording); `association` is reserved for a "
                "later linking pass. Prismic content was found to be a subset of PENDING "
                "and is therefore not ingested separately.",
        "images": {"top": [], "middle": [], "bottom": []},
        "audio": {"top": [], "middle": [], "bottom": []},
        "associations": [],
    }
    tmp = OUT / "_tmp"
    ...
    for part, items in IMAGES.items():
        ...
        for group, fid in items:
            ...
            manifest["images"][part].append({
                "id": f"{part}/{group}", "file": f"images/{part}/{group}.png",
                "group": group, "sourceDriveId": fid, "association": None,
            })
    ...
    for part, items in AUDIO.items():
        ...
        for group, fid, ext in items:
            ...
            manifest["audio"][part].append({
                "id": f"{part}/{group}", "file": f"audio/{part}/{group}.mp3",
                "group": group, "sourceDriveId": fid, "association": None,
            })
